...
global ARROW_PERSP _WARN ARROW_STRETCH _WARN ARROW_AXLIMITS
if isempty(ARROW_PERSP _WARN  ), ARROW_PERSP _WARN  =1; end;
if isempty(ARROW_STRETCH _WARN), ARROW_STRETCH _WARN=1; end;
...
(*% expand single-column arguments
o = ones(narrows 1);
if (size(start     ,1)==1),   start      = start(narrows 1)     ;   end;
...
while (any(axnotdone)),
        ...
        if ~arrow_WarpToFill(notstretched,manualcamera,curax),
                ...
                if 0 & ARROW_STRETCH _WARN,
                        ARROW_STRETCH _WARN = 0;
        ...
        if ARROW_PERSP _WARN & ~strcmp(get(curax,'Projection'),'orthographic'),
                ARROW_PERSP _WARN = 0;
...
% handle the clicks for one arrow
        ...
        global ARROW_CLICK _H ARROW_CLICK _PROP ARROW_CLICK _AX ARROW_CLICK _USE _Z
        ARROW_CLICK _H=H; ARROW_CLICK _PROP=prop; ARROW_CLICK _AX=ax;
        ARROW_CLICK _USE _Z=~arrow_is2DXY(ax)|~arrow_planarkids(ax);
        ...
        if lockStart & ~wasInterrupted,
                pt = arrow_point(ARROW_CLICK _AX,ARROW_CLICK _USE _Z);
                feval(mfilename,H,'Start',pt,'Stop',pt);
                set(H,'Visible','on');
                ARROW_CLICK _PROP='Stop';
                set(fig,'WindowButtonMotionFcn',[mfilename '(''callback'',''motion'');']);
...
% handle redrawing callbacks
        ...
        if strcmp(s,'motion'),
                % motion callback
                global ARROW_CLICK _H ARROW_CLICK _PROP ARROW_CLICK _AX ARROW_CLICK _USE _Z
                feval(mfilename,ARROW_CLICK _H,ARROW_CLICK _PROP,arrow_point(ARROW_CLICK _AX,ARROW_CLICK _USE _Z));
                drawnow;
        else,
